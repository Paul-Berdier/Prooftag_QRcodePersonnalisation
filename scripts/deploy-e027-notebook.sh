...
if [[ -n "$(git status --porcelain)" ]]; then
  ...
  echo "Commit/push/pull avant de déployer E027." >&2
  exit 1
fi

echo "===== E027 : API ET NOTEBOOK DU MÊME COMMIT ====="
notebook_replicas="$(
  kubectl get deployment prooftag-qr-notebook -n qr-core \
    -o jsonpath='{.spec.replicas}' 2>/dev/null || printf '0'
)"

# Construire et déployer les deux images avant de démarrer le runtime empêche
# Jupyter d'ouvrir un notebook d'un commit différent de celui exécuté par l'API.
bash scripts/deploy-app-image.sh
bash scripts/deploy-notebook-image.sh "notebooks/${notebook}"

started_by_this_script=0
rollback() {
  echo "Échec du démarrage E027." >&2
