...
kubectl -n "$namespace" exec "$pod" -c "$container" -- \
  python -c \
  "from pathlib import Path; import diffqrcoder, prooftag_qr; from prooftag_qr.lab import laboratory_profiles; path = Path(prooftag_qr.__file__).with_name('lab_static') / 'index.html'; text = path.read_text(encoding='utf-8'); assert '20260730-binary-target-1' in text; profiles = laboratory_profiles(); srpg = next(x for x in profiles if x['id'] == 'diffqrcoder_srpg'); srmpgd = next(x for x in profiles if x['id'] == 'diffqrcoder_srmpgd'); assert srpg['tools']['settings']['diffqrcoder_stage2_initialization'] == 'paper_stage1_noise'; assert 'diffqrcoder_qart_enabled' not in srpg['tools']['settings']; assert srmpgd['tools']['settings']['srmpgd_step_size'] == 1000.0; assert srmpgd['tools']['settings']['srmpgd_lpips_weight'] == 0.01; print('DiffQRCoder importé depuis', Path(diffqrcoder.__file__).resolve()); print('Stage 2 avec cible binaire exacte, SR-MPGD et assets Web confirmés')"
