...
kubectl -n "$namespace" exec "$pod" -c "$container" -- \
  python -c \
  "from pathlib import Path; import diffqrcoder, prooftag_qr; from prooftag_qr.qr import generate_diffqrcoder_qr; from prooftag_qr.validation import QRVerifyDecoder; root = Path(prooftag_qr.__file__).with_name('lab_static'); html = (root / 'index.html').read_text(encoding='utf-8'); js = (root / 'app.js').read_text(encoding='utf-8'); assert '20260805-e024-qr-verify-3' in html; assert 'Score QR-Verify' in js and 'CLIP-AES' not in js; payload = 'https://ptag.io/t/deploy-check'; decoder = QRVerifyDecoder(); attempts = decoder.decode_presets(generate_diffqrcoder_qr(payload).image); decoder.close(); assert len(attempts) == 37 and all(item['text'] == payload for item in attempts); print('DiffQRCoder', Path(diffqrcoder.__file__).resolve()); print('E024 QR-Verify confirmé: 37/37 exacts')"
