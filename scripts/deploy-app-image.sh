...
kubectl -n "$namespace" exec "$pod" -c "$container" -- \
  python -c \
  "from prooftag_qr.lab import laboratory_profiles; p = laboratory_profiles(); ids = [x['id'] for x in p]; expected = ['qr_reference', 'diffqrcoder_stage1', 'diffqrcoder_srpg', 'diffqrcoder_srmpgd', 'diffqrcoder_auto', 'diffqrcoder_srpg_s035', 'diffqrcoder_srpg_s050', 'diffqrcoder_srpg_s080', 'diffqrcoder_qart_srpg']; assert ids == expected, (ids, expected); generated = [x for x in p if x['backend'] == 'controlnet']; assert all(x['model'].get('diffqrcoder_upstream_enabled') for x in generated); print([(x['id'], x['output_variant'], x['enabled']) for x in p])"
kubectl -n "$namespace" exec "$pod" -c "$container" -- \
  python -c \
  "from pathlib import Path; import diffqrcoder, prooftag_qr; from prooftag_qr.lab import laboratory_profiles; path = Path(prooftag_qr.__file__).with_name('lab_static') / 'index.html'; text = path.read_text(encoding='utf-8'); assert '20260803-normalized-chain-1' in text; profiles = laboratory_profiles(); srpg = next(x for x in profiles if x['id'] == 'diffqrcoder_srpg'); srmpgd = next(x for x in profiles if x['id'] == 'diffqrcoder_srmpgd'); auto = next(x for x in profiles if x['id'] == 'diffqrcoder_auto'); qart = next(x for x in profiles if x['id'] == 'diffqrcoder_qart_srpg'); assert srpg['tools']['settings']['diffqrcoder_stage2_initialization'] == 'paper_stage1_noise'; assert srpg['tools']['settings']['diffqrcoder_stage2_target_mode'] == 'binary_exact'; assert srpg['tools']['settings']['diffqrcoder_stage2_strength'] == 0.65; assert auto['output_variant'] == 'auto' and auto['enabled'] is False; assert qart['enabled'] is False; assert srmpgd['tools']['settings']['srmpgd_max_initial_module_error_rate'] == 0.12; print('DiffQRCoder importé depuis', Path(diffqrcoder.__file__).resolve()); print('Validation normalisée, chaîne binaire, repli Stage 1 et garde couleur confirmés')"
