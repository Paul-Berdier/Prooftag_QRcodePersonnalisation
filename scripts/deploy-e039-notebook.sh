...
if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  echo "Ne pas sourcer ce script. Utiliser : bash scripts/deploy-e039-notebook.sh" >&2
  return 2
fi
set -Eeuo pipefail
...
[[ -f "notebooks/$notebook" ]] || { echo "Notebook E039 absent." >&2; exit 1; }
[[ -f prooftag_qr/e039_limiter_scanaware.py ]] || { echo "Runner E039 absent." >&2; exit 1; }
...
echo "===== E039 : build commit $git_sha ====="
...
[[ -n "$notebook_pod" ]] || { echo "Pod notebook E039 introuvable." >&2; exit 1; }
"$kubectl_bin" exec -n "$namespace" "$notebook_pod" -c notebook -- test -f "/workspace/notebooks/$notebook"
"$kubectl_bin" exec -n "$namespace" deployment/"$api_deployment" -c api -- python -c 'import prooftag_qr.e039_limiter_scanaware as m; assert len(m.DEFAULT_RECIPES)==10; assert m.E039Config().gamma==1000.0; print("E039 runtime import OK")'
...
echo "Calcul GPU   : bash scripts/run-e039-limiter-scanaware.sh"
echo "Après E039 TERMINÉ seulement : .\\scripts\\e039-remote.ps1"
