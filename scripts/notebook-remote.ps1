...
$pidFile = Join-Path `
    $env:TEMP `
    "prooftag-qr-notebook-tunnel.pid"

$tunnelLogFile = Join-Path `
    $env:TEMP `
    "prooftag-qr-notebook-ssh.log"
...
function Stop-RemoteNotebook {
    ...
    & ssh `
        $Server `
        "cd $RemoteRepository && bash scripts/notebook-server.sh stop"
...
function Assert-LocalPortAvailable {
    ...
    try {
        ...
        throw (
            "Le port local $LocalPort est deja occupe. " +
            "Utiliser par exemple -LocalPort 18889."
        )
...
if ($Stop) {
    ...
    Write-Host (
        "Notebook arrete et charge GPU precedente restauree."
    )
...
try {
    ...
    $remoteOutput = & ssh `
        $Server `
        "cd $RemoteRepository && bash scripts/notebook-server.sh $remoteAction $Notebook"


    if ($LASTEXITCODE -ne 0) {

        throw (
            "Impossible de demarrer le notebook distant."
        )
    ...
    $tokenLine = $remoteOutput |
        Where-Object {
            $_ -like "JUPYTER_TOKEN=*"
        } |
    ...
    if (-not $tokenLine) {
        ...
        throw (
            "Le serveur n'a pas retourne le jeton Jupyter."
        )

    }


    $token = $tokenLine.Substring(
        "JUPYTER_TOKEN=".Length
    )


    $targetLine = $remoteOutput |
        Where-Object {
            $_ -like "JUPYTER_TARGET=*"
        } |
        Select-Object -Last 1
    ...
    if (-not $targetLine) {
        ...
        throw (
            "Le serveur n'a pas retourne la cible reseau de Jupyter."
        )

    }


    $target = $targetLine.Substring(
        "JUPYTER_TARGET=".Length
    )


    if (
        $target -notmatch '^\d{1,3}(\.\d{1,3}){3}:\d+$'
    ) {

        throw (
            "Cible reseau Jupyter invalide : $target"
        )
    ...
    Write-Host (
        "Une fenetre SSH va s'ouvrir : saisir le mot de passe de $Server."
    )
    ...
    for (
        ...
        try {
            ...
            Invoke-WebRequest `
                -Uri "http://127.0.0.1:${LocalPort}/api" `
                -Headers $headers `
                -TimeoutSec 1 |
                Out-Null
            ...
            if (
                ...
                Write-Host (
                    "Mode campagne : Jupyter reste sur CPU " +
                    "et l'API conserve la RTX pour generer les donnees."
                )
            ...
            elseif (
                ...
                Write-Host (
                    "Mode analyse hors ligne : CPU uniquement, " +
                    "aucune generation et aucune charge GPU modifiee."
                )
                ...
                Write-Host (
                    "Mode generation directe : " +
                    "ce notebook utilise la RTX."
                )
            ...
            if ($Reset) {
                ...
                Write-Host (
                    "Tous les anciens kernels ont ete supprimes " +
                    "et la VRAM a ete liberee."
                )
            ...
            Write-Host (
                "Pour arreter et restaurer le GPU : " +
            ...
            if (
                ...
                throw (
                    "Le tunnel SSH s'est arrete avant " +
                    "l'ouverture de Jupyter. " +
                    "Journal : $tunnelLogFile"
                )
    ...
    throw (
        "Jupyter n'a pas repondu sur le port local $LocalPort."
    )
    ...
    if ($remoteStarted) {
        ...
        try {
            ...
            Write-Warning (
                "La restauration distante a aussi echoue : " +
                "$($_.Exception.Message)"
            )
