...
if [[ -n "$(git status --porcelain)" ]]; then
  ...
  echo "Commit/push/pull avant de déployer E028." >&2
  exit 1
fi

echo "===== E028 : API ET NOTEBOOK DU MÊME COMMIT ====="
notebook_replicas="$(
  kubectl get deployment prooftag-qr-notebook -n qr-core \
    -o jsonpath='{.spec.replicas}' 2>/dev/null || printf '0'
)"

# Les deux images immuables sont déployées avant le runtime Jupyter. Ainsi le
# notebook, l'API, les schémas d'export et l'audit d'appariement sont identiques.
bash scripts/deploy-app-image.sh
bash scripts/deploy-notebook-image.sh "notebooks/${notebook}"

started_by_this_script=0
rollback() {
  echo "Échec du démarrage E028." >&2
