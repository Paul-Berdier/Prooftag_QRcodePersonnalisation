...
$pidFile = Join-Path $env:TEMP "prooftag-qr-e038-tunnel.pid"
$logFile = Join-Path $env:TEMP "prooftag-qr-e038-ssh.log"

function Stop-E038Tunnel {
...
function Invoke-RemoteBash {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Execution Bash distante E038 en echec." }
    return $output
}

if ($Stop) {
    Stop-E038Tunnel
    Write-Host "Tunnel E038 arrete."
    exit 0
}
...
if [ "`$MODE" != "advisor-cpu" ]; then
  echo "E038 notebook mode invalide: `$MODE (attendu advisor-cpu)" >&2
  exit 1
fi
...
if (-not $tokenLine -or -not $targetLine) { throw "Token/cible Jupyter E038 absents." }
...
for ($attempt = 0; $attempt -lt 240; $attempt++) {
    ...
    try {
        ...
        Start-Process $url
        Write-Host "E038 ouvert : $url"
        Write-Host "Le notebook compare E035/E036 + 10 nouvelles recettes E038, images incluses."
        Write-Host "Pour fermer le tunnel : .\scripts\e038-remote.ps1 -Stop"
        exit 0
    }
    catch {
        if ($tunnel.HasExited) { throw "Tunnel SSH E038 arrete. Log: $logFile" }
    }
}
throw "Jupyter E038 n'a pas repondu sur le port $LocalPort."
