...
$Notebook = if ($Pipeline) { "38_e041_final_pipeline_visualizer.ipynb" } else { "37_e041_gamma_functional_frontier.ipynb" }
$pidFile = Join-Path $env:TEMP "prooftag-qr-e041-tunnel.pid"
...
if ($Stop) { Stop-Tunnel; Write-Host "Tunnel E041 arrete."; exit 0 }
Stop-Tunnel
...
RESULTS="`${PROOFTAG_E041_RESULTS_DIR:-/data/e041-gamma-functional-pattern-frontier-v1}"
kubectl scale deployment "`$DEP" -n "`$NS" --replicas=1 >/dev/null
kubectl rollout status deployment/"`$DEP" -n "`$NS" --timeout=1200s >/dev/null
kubectl exec -n "`$NS" deployment/"`$DEP" -c notebook -- test -f "`$RESULTS/verdict.json" || { echo "E041 verdict absent dans le PVC : `$RESULTS/verdict.json" >&2; exit 1; }
...
$out=& ssh $Server "printf '%s' '$b64' | base64 --decode | bash"; if($LASTEXITCODE -ne 0){throw "Verification distante E041 en echec."}
...
for($i=0;$i -lt 240;$i++){Start-Sleep -Milliseconds 500; try{Invoke-WebRequest "http://127.0.0.1:${LocalPort}/api" -Headers @{Authorization="token $token"} -TimeoutSec 1|Out-Null; Start-Process $url; Write-Host "E041 ouvert : $url"; exit 0}catch{if($p.HasExited){throw "Tunnel SSH E041 arrete."}}}
throw "Jupyter E041 ne repond pas."
